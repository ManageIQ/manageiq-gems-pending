class MiqSwiftStorage < MiqObjectStorage
  ...
  def initialize(settings)
    ...
    _scheme, _userinfo, @host, @port, _registry, path, _opaque, query, _fragment = URI.split(URI::RFC2396_PARSER.escape(@settings[:uri]))
  ...
  def uri_to_object_path(remote_file)
    ...
    object_file_with_bucket = URI.split(URI::RFC2396_PARSER.escape(remote_file))[5]
